import { Fragment, Teleport, Transition, TransitionGroup, computed, createBaseVNode, createVNode, effectScope, h, inject, mergeProps, nextTick, onBeforeUnmount, onMounted, onScopeDispose, provide, reactive, readonly, ref, resolveDynamicComponent, shallowRef, toRaw, toRef, toValue, useId, vShow, warn, watch, watchEffect, withDirectives } from "./vue.runtime.esm-bundler-CvPjzdzA.js";
import { CircularBuffer, IN_BROWSER, SUPPORTS_INTERSECTION, clamp, consoleError, convertToUnit, deepEqual, defer, destructComputed, eventName, genericComponent, getCurrentInstance, hasEvent, includes, isObject, isOn, keyCodes, keys, matchesSelector, omit, onlyDefinedProps, propsFactory, templateRef } from "./defineComponent-CAKzfwnn.js";
import { getForeground, isCssColor, isParsableColor, makeThemeProps, parseColor, provideTheme, useDisplay, useProxiedModel, useRtl, useToggleScope } from "./theme-8uHrIPrN.js";
import "/Users/palesoulmove/saas-soft-test/node_modules/vuetify/lib/directives/ripple/VRipple.css";
import "/Users/palesoulmove/saas-soft-test/node_modules/vuetify/lib/components/VOverlay/VOverlay.css";
import "/Users/palesoulmove/saas-soft-test/node_modules/vuetify/lib/components/VTooltip/VTooltip.css";
...
function nullifyTransforms(el) {
	...
	if (tx) {
		...
		const h$1 = sy ? rect.height / sy : el.offsetHeight + 1;
		return new Box({
			x,
			y,
			width: w,
...
const ripples = {
	...
	hide(el) {
		...
		const animation = ripples$1[ripples$1.length - 1];
		...
		setTimeout(() => {
			...
			setTimeout(() => {
				const ripples$2 = el.getElementsByClassName("v-ripple__animation");
				if (ripples$2.length === 1 && el.dataset.previousPosition) {
...
function keyboardRippleShow(e, keys$1) {
	if (!keyboardRipple && keys$1.includes(e.keyCode)) {
		keyboardRipple = true;
		rippleShow(e);
	}
}
...
function forwardRefs(target) {
	...
	return new Proxy(target, {
		get(target$1, key) {
			...
			for (const ref$1 of refs) if (ref$1.value && Reflect.has(ref$1.value, key)) {
				const val = Reflect.get(ref$1.value, key);
				return typeof val === "function" ? val.bind(ref$1.value) : val;
			}
		},
		has(target$1, key) {
			if (Reflect.has(target$1, key)) return true;
			if (typeof key === "symbol" || key.startsWith("$") || key.startsWith("__")) return false;
			for (const ref$1 of refs) if (ref$1.value && Reflect.has(ref$1.value, key)) return true;
			return false;
		},
		set(target$1, key, value) {
			if (Reflect.has(target$1, key)) return Reflect.set(target$1, key, value);
			if (typeof key === "symbol" || key.startsWith("$") || key.startsWith("__")) return false;
			for (const ref$1 of refs) if (ref$1.value && Reflect.has(ref$1.value, key)) return Reflect.set(ref$1.value, key, value);
			return false;
		},
		getOwnPropertyDescriptor(target$1, key) {
			...
			for (const ref$1 of refs) {
				if (!ref$1.value) continue;
				const descriptor$1 = getDescriptor(ref$1.value, key) ?? ("_" in ref$1.value ? getDescriptor(ref$1.value._?.setupState, key) : void 0);
				if (descriptor$1) return descriptor$1;
			}
			for (const ref$1 of refs) {
				const childRefs = ref$1.value && ref$1.value[Refs];
				if (!childRefs) continue;
				const queue = childRefs.slice();
				while (queue.length) {
					const ref$2 = queue.shift();
					const descriptor$1 = getDescriptor(ref$2.value, key);
					if (descriptor$1) return descriptor$1;
					const childRefs$1 = ref$2.value && ref$2.value[Refs];
